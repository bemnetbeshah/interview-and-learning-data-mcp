...
The included [Dockerfile](./Dockerfile) runs the server with `MCP_TRANSPORT=streamable-http`, binds to `0.0.0.0`, and stores the SQLite database under `/data`. On Railway, mount a Railway Volume at `/data` rather than using a Dockerfile `VOLUME` instruction.
